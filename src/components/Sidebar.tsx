...
export default function Sidebar() {
  ...
  // Animated desktop sidebar content (first visit only)
  const animatedSidebarContent = (
    ...
    >
      ...
      {/* Location */}
      <motion.div variants={fadeUpItem} style={{ fontSize: 12, textTransform: "uppercase", lineHeight: 1.2 }}>
        Los Angeles<br />
        CA, USA (GMT-8)
      </motion.div>
  ...
  return (
    <>
      ...
      <aside className={`sidebar sidebar-desktop${isHome ? " sidebar-home" : ""}`}>
        {isFirstVisit ? animatedSidebarContent : sidebarContent}
      </aside>

      {/* Mobile header */}
      <div
        className={`mobile-header${isHome ? " mobile-header-home" : ""}`}
      ...
      >
        ...
        {/* Hamburger button - always visible, transforms to X */}
        <button
